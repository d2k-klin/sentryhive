...
description: >
  ...
  cloudsplaining, hardeneks, automated-security-helper) plus aws-cli and
...
SentryHive bundles four OSS scanner CLIs plus aws-cli and kubectl, pinned as
`ARG` lines at the top of `./Dockerfile` (`PROWLER_VERSION`,
`CLOUDSPLAINING_VERSION`, `HARDENEKS_VERSION`, `ASH_VERSION`,
`AWSCLI_VERSION`, `KUBECTL_VERSION`). Each tool's CLI output is parsed by a
matching wrapper in `sentryhive/scanners/` (`prowler.py`, `cloudsplaining.py`,
`hardeneks.py`, `ash.py`) and normalized in `sentryhive/normalize.py`.

1. Read the current pins from the Dockerfile.
2. For each tool, find the latest released version:
   - prowler, cloudsplaining, hardeneks, automated-security-helper: PyPI JSON
     API (`https://pypi.org/pypi/<name>/json` → `info.version`)
...
4. Otherwise, for each outdated tool, read its release notes / changelog
   ...
   compatibility between the four tools installed into the same venv.
5. Bump the `ARG` pin(s) in the Dockerfile. If a release changed something
