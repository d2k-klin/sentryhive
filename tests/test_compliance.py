...
def test_finding_frameworks_set():
    f = Finding(tool="prowler", check="c", title="t", description="d",
...
def test_compliance_posture_counts_pass_and_fail():
    findings = [
        Finding(tool="prowler", check="a", title="t", description="d", status="fail",
                resource="r1", compliance_refs=["CIS:1.1"]),
        Finding(tool="prowler", check="b", title="t", description="d", status="pass",
                resource="r2", compliance_refs=["CIS:1.2"]),
        Finding(tool="prowler", check="c", title="t", description="d", status="pass",
                resource="r3", compliance_refs=["PCI-DSS:8.3"]),
    ]
...
def test_iam_highlights_surfaces_priv_esc():
    findings = [
        Finding(tool="cloudsplaining", check="PrivilegeEscalation", title="priv esc: Role",
                description="d", severity=Severity.HIGH, service="iam", resource="Role", status="fail"),
        Finding(tool="prowler", check="s3_public", title="public bucket", description="d",
...
def test_build_report_populates_consultant_fields():
    results = [ScanResult("prowler", ScanStatus.OK, version="prowler 4.0", findings=[
        Finding(tool="prowler", check="a", title="t", description="d", status="fail",
                severity=Severity.HIGH, resource="r1", compliance_refs=["CIS:1.1"]),
    ])]
    report = build_report(results, account_id="123", identity_arn="arn", regions=["us-east-1"],
